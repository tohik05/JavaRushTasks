...
public class Solution {
    ...
    public static boolean isWeekend(LocalDateTime dateTime) {
        int temp = dateTime.getDayOfWeek().getValue();

        if (FRIDAY == temp && dateTime.getHour() >= WEEKEND_START_FRIDAY_CUT_OFF_HOUR) {
            return true;
        }
        if (SATURDAY == temp) {
            return true;
        }
        if (SUNDAY == temp && dateTime.getHour() < WEEKEND_END_SUNDAY_CUT_OFF_HOUR) {
            return true;
        }
        return false;
    }
